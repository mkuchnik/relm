...
def _is_low_level_api(query: query_type) -> bool:
    """Return true if query is low-level API."""
    return isinstance(query, SearchQuery)


def _inspect_query(query: query_type) -> None:
    """Issue warnings for bad queries."""
    if _is_low_level_api(query):
        logger.warning("Query is using low-level API."
                       " Most users should use SimpleSearchQuery API.")
...
def prepare(model, tokenizer, prepare_options: PrepareOptions = None):
    ...
    if prepare_options.manage_device_placement:
        ...
        model = model.eval()
    elif prepare_options.manage_device_placement is None:
        is_model_using_cuda = next(model.parameters()).is_cuda
        if torch.cuda.is_available() and not is_model_using_cuda:
            logger.warning("Model running on CPU when CUDA available")
