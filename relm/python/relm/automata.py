...
from relm.priority_queue import PriorityQueue
...
def _is_cyclic_graph(g: dict) -> bool:
    """Return whether graph g contains a cycle.

    Assumes g is an adjacency list representation where dict nodes are keys and
    neighbors are the value.
    """
    visiting = set()
    visited = set()
...
def dijkstra_from_automata(automata, expand_neighbor_costs_fn,
    ...
    # Loosely based off of the implementation at:
...
def accepted(output_apply) -> bool:
...
def apply_fst_accepted(elements, automata_op) -> bool:
    """Return True if elements is accepted by automata."""
    automata = automata_from_token_list(elements)
    symbol_table = automata_op.input_symbols()
    if symbol_table is not None:
        # NOTE(mkuchnik): Avoid expensive copies here by re-using the table.
        attach_symbol_table(automata, symbol_table)
    ret = fst.compose(automata, automata_op)
    ret = accepted(ret)
    return ret
